...
echo "Step: RCON configuration — checking environment variables..."
if [ -n "${RCON_PASSWORD_ENV}" ]; then
	echo "RCON: applying configuration (port=${RCON_PORT_ENV})..."
	...
	echo "RCON enabled in server.properties for server (rcon.port=${RCON_PORT_ENV})."
else
	echo "RCON: not configured (RCON_PASSWORD not set)."
fi
...
if [ -x "scripts/start_rcon_forwarder.sh" ]; then
	echo "Starting RCON forwarder helper (scripts/start_rcon_forwarder.sh)..."
...
if [ ! -f minecraft_server.jar ]; then
	echo "Step: checking for minecraft_server.jar in $(pwd)..."
...
echo "Starting up to Grimoire Server..."
echo "Step: launching JVM (Xms=6G, Xmx=6G) ..."
...
java_pid=$!
echo "JVM launched (pid=${java_pid}); output -> start_out.log"
