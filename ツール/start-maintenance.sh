...
echo "Step: backing up current configuration to ${BACKUP_DIR}..."
...
echo "Backup: server.properties and whitelist.json copied to ${BACKUP_DIR} (timestamp=${TIMESTAMP})"

# Set whitelist=true and update motd
echo "Step: enabling whitelist and updating MOTD..."
...
PY
  ...
  echo "Admin additions to whitelist completed (requested: ${ADD_ADMINS})"
fi
...
echo "Merging ops.json into whitelist.json to ensure ops are whitelisted..."
...
if [ -x "scripts/start_rcon_forwarder.sh" ]; then
  echo "Starting RCON forwarder helper (maintenance)..."
